            ...
            for (int p = 0; p < iconsCount; p++) {
                ...
                if (iconVisibleInMiniMap) {
                    ...
                    if (iconVisibleInMiniMap) {
                        ...
                        // Scale icon
                        poi.miniMapCurrentIconScale = _miniMapIconSize * poi.miniMapIconScale * miniMapIconTempScale;
                        Vector3 rtCurrentScale = poi.miniMapIconImageRT.localScale;
                        if (SignificantlyDifferents(poi.miniMapCurrentIconScale, rtCurrentScale.x)) {
                            needUpdateMiniMapIcons = true;
                            float scale = Mathf.Lerp(rtCurrentScale.x, poi.miniMapCurrentIconScale, scaleAnimSpeed);
                            scaleVector.x = scaleVector.y = scale;
                            poi.miniMapIconImageRT.localScale = scaleVector;
                        }
                        ...
                        if (poi.miniMapShowRotation) {
                            ...
                            poi.miniMapIconRT.localEulerAngles = rot;
                        }
#if UNITY_EDITOR
else {
// in editor, cancel any icon rotation when disabling the "miniMapShowRotation" option
    poi.miniMapIconRT.localRotation = Quaternion.identity;
}
#endif
                    }
                }
