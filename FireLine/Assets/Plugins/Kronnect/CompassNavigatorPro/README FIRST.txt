...
*    (C) Copyright 2023-2024 Kronnect   * 
*              README FILE              *
*****************************************
...
Version 2.7.1
- [Fix] Fixed some resource filename references
- [Fix] Fixed recursive call in onScreenIndicatorAlpha property

Version 2.7
- API: added OnMiniMapBeforeCapture/OnMiniMapAfterCapture events

Version 2.6.2
- [Fix] Fixed compass bar icon prefab loading issue

Version 2.6.1
- API: added static method CompassPro.Create() to easily add the asset to any scene at runtime
- [Fix] Fixed minimap player compass rotation when using world-space canvas mode

Version 2.6
- MiniMap: added full screen mode max drag distance setting
- MiniMap: added auto reset drag option
- Offscreen indicators: added custom margin override option
- [Fix] Fixed on-screen indicators visibility issue

Version 2.5
- Radar variant: option to use custom texture instead of procedural rings
- Added custom POI scale multiplier when clamped in minimap
- Added "Fade POIs" option to Compass Navigator POIs inspector section (used in radar mode)
- CHANGE: Clamped POIs no longer fade out in radar mode
- CHANGE: screen indicators scale internal calculation has been changed. You may need to reduce the scale value in the inspector.
- POI id assignment improvements

Version 2.4
- Added 2 new compass styles with integrated degree and tick marks
- Added "Scroll Offset" option
- POI: added "Icon Fixed Scale" option

Version 2.3
- Added "Hide If No Camera" option. This option will hide or make the compass visible until a valid camera is present in the scene.
- Added support for pooled objects

Version 2.2.1
- [Fix] Fixed icons position issue in maximized mode when keep aspect ratio option is disabled

Version 2.2
- Indicators: added "Max Visible Distance" option
- Indicators: added "Display Rect" option
- Added MiniMapScrollWheel script to demo scene to illustrate how to zoom in/out using mouse wheel using the API
- [Fix] Fixed POIs restricted to secondary radar map not shown
- [Fix] Fixed radar pulse falloff setting issue

Version 2.1.1
- Added "Never" option to Title Visibility setting
- [Fix] Fixed custom POI screen indicator scale being ignored

Version 2.1
- Added support for scrollable compass bar (select Style = Custom and enable the new Scroll setting)

Version 2.0
- Smoother zoom in/out operations
- POI: added "Allowed Minimap" setting (Any, MiniMapOnly, RadarOnly). Useful to restrict a POI to a specific mini-map type in case of using two Compass instances.
