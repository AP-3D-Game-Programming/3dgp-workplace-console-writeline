Shader "CompassNavigatorPro/MiniMapOverlayUnlit"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			#pragma multi_compile_local _ COMPASS_ROTATE_BORDER
			...
			#if COMPASS_ROTATED || COMPASS_ROTATE_BORDER
			...
			{
				float2 borderUV = IN.texcoord;
				#if COMPASS_ROTATE_BORDER
					borderUV = IN.rotatedUV;
				#endif
				fixed4 border = (tex2D(_BorderTex, borderUV) + _TextureSampleAdd) * IN.color;
				fixed4 mask = tex2D(_MaskTex, borderUV);
